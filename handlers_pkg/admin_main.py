...
def show_settings(chat_id):
    ...
    markup.add(
        types.InlineKeyboardButton("📣 Channel Panel", callback_data="ch_open"),
        types.InlineKeyboardButton("🏧 WD Control", callback_data="wd_open"),
    )
    safe_send(
        chat_id,
...
# ======================== CHANNEL PANEL ========================
@bot.message_handler(func=lambda m: m.text == "📣 Channel Panel" and is_admin(m.from_user.id))
def admin_channel_panel(message):
    show_channel_panel(message.chat.id)


def format_channel_row(idx, row):
    mode = "FORCE" if row.get("force_join") else "OPEN"
    status = "ON" if row.get("active") else "OFF"
    return f"{idx}. <b>{h(row.get('label') or 'Join')}</b> • {mode} • {status}\nURL: <code>{h(row.get('url') or '-')}</code>\nChat ID: <code>{row.get('chat_id') or 0}</code>"


def format_extra_button_row(idx, row):
    status = "ON" if row.get("active") else "OFF"
    return f"{idx}. <b>{h(row.get('label') or 'Button')}</b> • {h(row.get('type') or 'url')} • {status}\nValue: <code>{h(row.get('value') or '-')}</code>"


def show_channel_panel(chat_id):
    channels = get_join_channels()
    extras = get_extra_join_buttons()
    request_url = get_join_request_url()
    join_image = get_join_image()
    text = (
        f"{pe('megaphone')} <b>Channel & Button Manager</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{pe('link')} <b>Main Request URL:</b> <code>{h(request_url or '-')}</code>\n"
        f"🖼 <b>Join Image:</b> <code>{h(join_image or '-')}</code>\n"
        f"{pe('shield')} <b>Join Channels:</b> {len(channels)}\n"
        f"{pe('tag')} <b>Extra Buttons:</b> {len(extras)}\n\n"
    )
    if channels:
        text += f"<b>Join Channel List</b>\n"
        for idx, row in enumerate(channels[:8], 1):
            text += format_channel_row(idx, row) + "\n\n"
    else:
        text += "No join channels configured yet.\n\n"
    if extras:
        text += f"<b>Extra Button List</b>\n"
        for idx, row in enumerate(extras[:6], 1):
            text += format_extra_button_row(idx, row) + "\n\n"
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("➕ Add Join Channel", callback_data="ch_add"),
        types.InlineKeyboardButton("➕ Add Extra Button", callback_data="xb_add"),
    )
    markup.add(
        types.InlineKeyboardButton("🧩 Manage Join Channels", callback_data="ch_manage"),
        types.InlineKeyboardButton("🔘 Manage Extra Buttons", callback_data="xb_manage"),
    )
    markup.add(
        types.InlineKeyboardButton("🔗 Set Main URL", callback_data="ch_set_request"),
        types.InlineKeyboardButton("🖼 Set Join Image", callback_data="ch_set_image"),
    )
    markup.add(
        types.InlineKeyboardButton("👀 Preview Join Panel", callback_data="ch_preview"),
        types.InlineKeyboardButton("⚙️ Settings", callback_data="open_settings"),
    )
    safe_send(chat_id, text[:4000], reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data == "ch_open")
def ch_open(call):
    if not is_admin(call.from_user.id):
        return
    safe_answer(call)
    show_channel_panel(call.message.chat.id)


@bot.callback_query_handler(func=lambda call: call.data == "ch_add")
def ch_add(call):
    settings_ask(call, "admin_add_join_channel", f"{pe('pencil')} Send:\n<code>Label | URL | CHAT_ID | force/open | on/off</code>\nExample:\n<code>Main Channel | https://t.me/mychannel | -1001234567890 | force | on</code>")


@bot.callback_query_handler(func=lambda call: call.data == "xb_add")
def xb_add(call):
    settings_ask(call, "admin_add_extra_button", f"{pe('pencil')} Send:\n<code>Label | url/callback | VALUE | on/off</code>\nExample URL:\n<code>Visit Website | url | https://example.com | on</code>\nExample callback:\n<code>Open Refer | callback | open_refer | on</code>")


@bot.callback_query_handler(func=lambda call: call.data == "ch_set_request")
def ch_set_request(call):
    settings_ask(call, "admin_set_join_request_url", f"{pe('pencil')} Send new main request URL or channel invite link:")


@bot.callback_query_handler(func=lambda call: call.data == "ch_set_image")
def ch_set_image(call):
    settings_ask(call, "admin_set_join_image", f"{pe('pencil')} Send new join image URL:")


@bot.callback_query_handler(func=lambda call: call.data == "ch_preview")
def ch_preview(call):
    if not is_admin(call.from_user.id):
        return
    safe_answer(call, "Preview sent")
    send_join_message(call.message.chat.id)


@bot.callback_query_handler(func=lambda call: call.data == "ch_manage")
def ch_manage(call):
    if not is_admin(call.from_user.id):
        return
    safe_answer(call)
    channels = get_join_channels()
    if not channels:
        safe_send(call.message.chat.id, f"{pe('info')} No join channels configured yet.")
        return
    markup = types.InlineKeyboardMarkup(row_width=1)
    for idx, row in enumerate(channels, 1):
        label = row.get("label") or f"Channel {idx}"
        mode = "F" if row.get("force_join") else "O"
        status = "🟢" if row.get("active") else "🔴"
        markup.add(types.InlineKeyboardButton(f"{status} {idx}. {label} [{mode}]", callback_data=f"ch_item|{idx-1}"))
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="ch_open"))
    safe_send(call.message.chat.id, f"{pe('list')} <b>Select a join channel to manage</b>", reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith("ch_item|"))
def ch_item(call):
    if not is_admin(call.from_user.id):
        return
    safe_answer(call)
    try:
        ...
        safe_send(call.message.chat.id, f"{pe('cross')} Invalid selection")
        return
    channels = get_join_channels()
    if idx < 0 or idx >= len(channels):
        safe_send(call.message.chat.id, f"{pe('cross')} Channel not found")
        return
    row = channels[idx]
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("✏️ Edit", callback_data=f"ch_edit|{idx}"),
        types.InlineKeyboardButton("🔁 Toggle", callback_data=f"ch_toggle|{idx}"),
    )
    markup.add(
        types.InlineKeyboardButton("🗑 Delete", callback_data=f"ch_delete|{idx}"),
        types.InlineKeyboardButton("🔙 Back", callback_data="ch_manage"),
    )
    safe_send(call.message.chat.id, format_channel_row(idx + 1, row), reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith("ch_edit|"))
def ch_edit(call):
    if not is_admin(call.from_user.id):
        return
    try:
        ...
        return
    safe_answer(call)
    set_state(call.from_user.id, "admin_edit_join_channel", {"index": idx})
    safe_send(call.message.chat.id, f"{pe('pencil')} Send updated value:\n<code>Label | URL | CHAT_ID | force/open | on/off</code>")


@bot.callback_query_handler(func=lambda call: call.data.startswith("ch_toggle|"))
def ch_toggle(call):
    if not is_admin(call.from_user.id):
        return
    try:
    ...
    if idx < 0 or idx >= len(channels):
        ...
        return
    channels[idx]["active"] = not bool(channels[idx].get("active"))
    save_join_channels(channels)
    log_admin_action(call.from_user.id, "toggle_join_channel", f"Join channel #{idx+1} active={channels[idx]['active']}")
    safe_answer(call, "Updated")
    show_channel_panel(call.message.chat.id)


@bot.callback_query_handler(func=lambda call: call.data.startswith("ch_delete|"))
def ch_delete(call):
    if not is_admin(call.from_user.id):
        return
    try:
    ...
    save_join_channels(channels)
    log_admin_action(call.from_user.id, "delete_join_channel", f"Deleted join channel {removed.get('label')}")
    safe_answer(call, "Deleted")
    show_channel_panel(call.message.chat.id)


@bot.callback_query_handler(func=lambda call: call.data == "xb_manage")
def xb_manage(call):
    if not is_admin(call.from_user.id):
        return
    safe_answer(call)
    ...
    if not buttons:
        safe_send(call.message.chat.id, f"{pe('info')} No extra buttons configured yet.")
        return
    markup = types.InlineKeyboardMarkup(row_width=1)
    for idx, row in enumerate(buttons, 1):
        status = "🟢" if row.get("active") else "🔴"
        markup.add(types.InlineKeyboardButton(f"{status} {idx}. {row.get('label')}", callback_data=f"xb_item|{idx-1}"))
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="ch_open"))
    safe_send(call.message.chat.id, f"{pe('list')} <b>Select an extra button to manage</b>", reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith("xb_item|"))
def xb_item(call):
    if not is_admin(call.from_user.id):
        return
    safe_answer(call)
    ...
    if idx < 0 or idx >= len(buttons):
        safe_send(call.message.chat.id, f"{pe('cross')} Button not found")
        return
    row = buttons[idx]
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("✏️ Edit", callback_data=f"xb_edit|{idx}"),
        types.InlineKeyboardButton("🔁 Toggle", callback_data=f"xb_toggle|{idx}"),
    )
    markup.add(
        types.InlineKeyboardButton("🗑 Delete", callback_data=f"xb_delete|{idx}"),
        types.InlineKeyboardButton("🔙 Back", callback_data="xb_manage"),
    )
    safe_send(call.message.chat.id, format_extra_button_row(idx + 1, row), reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith("xb_edit|"))
def xb_edit(call):
    if not is_admin(call.from_user.id):
        return
    try:
        ...
        return
    safe_answer(call)
    set_state(call.from_user.id, "admin_edit_extra_button", {"index": idx})
    safe_send(call.message.chat.id, f"{pe('pencil')} Send updated value:\n<code>Label | url/callback | VALUE | on/off</code>")


@bot.callback_query_handler(func=lambda call: call.data.startswith("xb_toggle|"))
def xb_toggle(call):
    if not is_admin(call.from_user.id):
        return
    try:
    ...
    if idx < 0 or idx >= len(buttons):
        safe_answer(call, "Not found", True)
        return
    buttons[idx]["active"] = not bool(buttons[idx].get("active"))
    save_extra_join_buttons(buttons)
    log_admin_action(call.from_user.id, "toggle_extra_button", f"Extra button #{idx+1} active={buttons[idx]['active']}")
    safe_answer(call, "Updated")
    show_channel_panel(call.message.chat.id)


@bot.callback_query_handler(func=lambda call: call.data.startswith("xb_delete|"))
def xb_delete(call):
    if not is_admin(call.from_user.id):
        return
    try:
    ...
    save_extra_join_buttons(buttons)
    log_admin_action(call.from_user.id, "delete_extra_button", f"Deleted extra button {removed.get('label')}")
    safe_answer(call, "Deleted")
    show_channel_panel(call.message.chat.id)


# ======================== WITHDRAW CONTROL PANEL ========================
@bot.message_handler(func=lambda m: m.text == "🏧 WD Control" and is_admin(m.from_user.id))
def admin_wd_control(message):
    show_wd_control(message.chat.id)


@bot.callback_query_handler(func=lambda call: call.data == "wd_open")
def wd_open(call):
    if not is_admin(call.from_user.id):
        return
    safe_answer(call)
    show_wd_control(call.message.chat.id)


def show_wd_control(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton(f"Min ₹{get_setting('min_withdraw')}", callback_data="wd_set_min"),
        types.InlineKeyboardButton(f"Max ₹{get_setting('max_withdraw_per_day')}", callback_data="wd_set_max"),
    )
    markup.add(
        types.InlineKeyboardButton(f"Single ₹{get_setting('max_single_withdraw_amount')}", callback_data="wd_set_single"),
        types.InlineKeyboardButton(f"Daily Count {withdraw_limit.get_daily_limit()}", callback_data="wd_set_daily_count"),
    )
    markup.add(
        types.InlineKeyboardButton(f"Refs {get_setting('withdraw_required_referrals')}", callback_data="wd_set_refs"),
        types.InlineKeyboardButton(f"Hours {get_setting('withdraw_time_start')}-{get_setting('withdraw_time_end')}", callback_data="wd_set_time"),
    )
    markup.add(
        types.InlineKeyboardButton(f"{'🟢' if bool(get_setting('withdraw_enabled')) else '🔴'} Withdraw", callback_data="tog_withdraw"),
        types.InlineKeyboardButton(f"{'🟢' if bool(get_setting('withdraw_referral_requirement_enabled')) else '🔴'} Referral Gate", callback_data="tog_wd_ref_gate"),
    )
    markup.add(
        types.InlineKeyboardButton("🧾 Redeem Withdraw Settings", callback_data="rm_settings"),
        types.InlineKeyboardButton("⚙️ Main Settings", callback_data="open_settings"),
    )
    safe_send(chat_id, f"{pe('wallet')} <b>Withdrawal Control Panel</b>\n━━━━━━━━━━━━━━━━━━━━━━\n\n{withdraw_limit.build_limit_status_block(ADMIN_ID)}\n\nMin withdraw: <b>₹{get_setting('min_withdraw')}</b>\nMax withdraw/day amount: <b>₹{get_setting('max_withdraw_per_day')}</b>\nSingle withdraw cap: <b>₹{get_setting('max_single_withdraw_amount')}</b>\nRequired referrals: <b>{get_setting('withdraw_required_referrals')}</b>", reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data == "wd_set_min")
def wd_set_min(call):
    settings_ask(call, "admin_set_min_withdraw", f"{pe('pencil')} Enter new minimum withdrawal amount:")


@bot.callback_query_handler(func=lambda call: call.data == "wd_set_max")
def wd_set_max(call):
    settings_ask(call, "admin_set_max_withdraw", f"{pe('pencil')} Enter new maximum withdrawal amount per day:")


@bot.callback_query_handler(func=lambda call: call.data == "wd_set_single")
def wd_set_single(call):
    settings_ask(call, "admin_set_max_single_withdraw", f"{pe('pencil')} Enter new single withdrawal cap:")


@bot.callback_query_handler(func=lambda call: call.data == "wd_set_daily_count")
def wd_set_daily_count(call):
    settings_ask(call, "admin_set_daily_withdraw_limit", f"{pe('pencil')} Enter allowed number of withdrawals per day:")


@bot.callback_query_handler(func=lambda call: call.data == "wd_set_refs")
def wd_set_refs(call):
    settings_ask(call, "admin_set_withdraw_required_referrals", f"{pe('pencil')} Enter required referrals for withdrawal:")


@bot.callback_query_handler(func=lambda call: call.data == "wd_set_time")
def wd_set_time(call):
    settings_ask(call, "admin_set_withdraw_time", f"{pe('pencil')} Enter withdraw time range:\n<code>START-END</code>")
